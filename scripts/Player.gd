...
func _network_process(input: Dictionary) -> void:
	...
	# delta vector
	var delta = SGFixed.vector2(0, 0)
	# delta.y += gravity
	delta = delta.add(input_vector.mul(speed))
	

	# update position based delta vector // position += delta
	fixed_position = fixed_position.add(delta)
