...
# LOBBY EXAMPLE
#################################################
onready var LOBBY_MEMBER = preload("res://LobbyMember.tscn/")
onready var BUTTON_THEME = preload("res://button-theme.tres")
var LOBBY_ID: int = 0
...
func _leave_Lobby() -> void:
	...
	if LOBBY_ID != 0:
		...
		$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] Leaving lobby "+str(LOBBY_ID)+".\n")
...
func _on_Lobby_Created(connect: int, lobby_id: int) -> void:
	if connect == 1:
		$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] Created a lobby: "+str(LOBBY_ID)+"\n")
...
func _on_Lobby_Joined(lobby_id: int, _permissions: int, _locked: bool, response: int) -> void:
	...
	if response == 1:
		...
		$Frame/Main/Displays/Outputs/Titles/Lobby.set_text("Lobby ID: "+str(LOBBY_ID))
		# Append to output
		$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] Joined lobby "+str(LOBBY_ID)+".\n")
	...
	# Else it failed for some reason
		...
		$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] Failed joining lobby "+str(lobby_id)+": "+str(FAIL_REASON)+"\n")
...
func _make_P2P_Handshake() -> void:
	$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] Sending P2P handshake to the lobby...\n")
...
func _on_P2P_Session_Request(remoteID: int) -> void:
	...
	$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] P2P session request from "+str(REQUESTER)+"\n")
	# Accept the P2P session; can apply logic to deny this request if needed
	var SESSION_ACCEPTED: bool = Steam.acceptP2PSessionWithUser(remoteID)
	$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] P2P session was connected: "+str(SESSION_ACCEPTED)+"\n")
	# Make the initial handshake
	_make_P2P_Handshake()
...
func _send_Test_Info() -> void:
	$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] Sending test packet data...\n")
...
func _read_P2P_Packet() -> void:
	...
	if PACKET_SIZE > 0:
		...
		$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] Packet from "+str(PACKET_SENDER)+": "+str(READABLE)+"\n")
		# Append logic here to deal with packet data
		if READABLE['message'] == "start":
			$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] Starting P2P game...\n")
...
func _send_P2P_Packet(target: int, packet_data: Dictionary) -> void:
	...
	$Frame/Main/Displays/Outputs/Output.append_bbcode("[STEAM] P2P packet sent successfully? "+str(SEND_RESPONSE)+"\n")
